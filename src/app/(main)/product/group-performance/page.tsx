...
const GrooupPerformancePage: React.FC = () => {
  ...
  const { sales: colorSales, loading: colorLoading } = useAppSelector((state) => state.color);
  const { sales: priceSales, loading: priceLoading } = useAppSelector((state) => state.price);
  ...
  const handleRowClick = (record: SalesData) => {
    ...
    dispatch(
      fetchSalesDataDetail({
        awal: selectedRange[0] ? selectedRange[0].format('YYYY-MM-DD') : '2023-01-01',
        akhir: selectedRange[1] ? selectedRange[1].format('YYYY-MM-DD') : '2023-12-31',
        limit: 100,
        group: 'group',
  ...
  const filteredSalesDetail = salesDetail?.filter((item) => item.artikel.toLowerCase().includes(searchTerm.toLowerCase()));
    ...
    <Suspense>
      <div>
        ...
        <TableColorPage
          ...
          kdtoko={selectedSites}
          awal={Array.isArray(selectedRange) && selectedRange.length > 0 && selectedRange[0] ? selectedRange[0].format('YYYY-MM-DD') : '2023-01-01'} // Kirim nilai awal
          akhir={Array.isArray(selectedRange) && selectedRange.length > 1 && selectedRange[1] ? selectedRange[1].format('YYYY-MM-DD') : '2023-12-31'} // Kirim nilai akhir
        />
        <Modal
