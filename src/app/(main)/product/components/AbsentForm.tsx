...
const AbsentForm = () => {
  ...
  const filteredSites = sitedata.filter((site) => selectedChannels.includes(site.category)).filter((site) => site.name.toLowerCase().includes(searchTerm.toLowerCase()));
    ...
    <div>
      <div className='flex gap-4 w-full'>
              ...
              <Row gutter={[0, 8]}>
                {channelOptions.map((option) => (
                  <Col span={24} key={option}>
                    <div className={`${selectedChannels.includes(option) ? 'bg-blue-100' : ''} p-2 rounded`} style={{ width: '100%' }}>
        ...
        <div className='mt-4 bg-white rounded-xl p-7 w-3/4'>
          ...
          <Input placeholder='Search sites...' onChange={handleSearchInputChange} style={{ marginBottom: '16px' }} />
